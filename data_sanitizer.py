...
def clean_parquet_rosetta(src: Path, fout: TextIO, batch_size: int) -> int:
    """Rosetta parquet: C/Rust by task_name, NBSP removed, strip; require both snippets.
    Writes JSONL to ``fout``. Returns number of lines written."""
    ...
    for task_name in sorted(by_task.keys()):
        ...
        record = _normalize_record_descriptor(
            {"task_name": task_name, "c_code": c_code, "safe_rust_code": rust_code}
        )
...
def _dedup_key(obj: dict) -> tuple:
    ...
    return (obj.get("c_code"), obj.get("safe_rust_code"))
    ...
    *,
    ...
    with src.open(encoding="utf-8") as fin:
        for line in fin:
            ...
            # Extract and remove the old keys
            c_raw = obj.pop("C")
            r_raw = obj.pop("Rust")
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            ...
            "Outputs are merged into a single final_dataset.jsonl file under CLEANED_DATASETS_DIR."
        ),
    )
    p.add_argument(
        "--batch-size",
...
def main(argv: list[str] | None = None) -> int:
    ...
    final_out_path = dest_root / "final_dataset_v2.jsonl"
    
    print(f"Source: {source_root}\nDestination: {final_out_path}\n")

    total_files = 0
    seen_jsonl: set[tuple] = set()

    with final_out_path.open("w", encoding="utf-8", newline="\n") as fout:
        for dataset_dir in subdirs:
            ...
            for src in files:
                ...
                if suf == ".parquet":
                    n = clean_parquet_rosetta(src, fout, args.batch_size)
                    print(
                        f"{rel_name}/{src.name} [parquet -> merged]: "
                        f"{n} lines added"
                    )
                elif suf == ".jsonl":
                    drop_keys = (
                    ...
                    lines_read, with_c_rust, written = clean_jsonl_dedup(
                        src, fout, seen_jsonl, drop_keys=drop_keys
                    )
                    dups = with_c_rust - written
                    print(
                        f"{rel_name}/{src.name} [jsonl dedup -> merged]: "
                        f"{lines_read} lines read, {with_c_rust} with both C and Rust, "
                        f"{written} unique added"
                        + (f", {dups} global duplicates dropped" if dups else "")
                    )
