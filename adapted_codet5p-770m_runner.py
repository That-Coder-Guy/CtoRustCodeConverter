...
LORA_ADAPTER = os.path.join(_TRAINING_DIR, "training_results_codet5p-770m", "final_model")
...
if not os.path.isdir(LORA_ADAPTER):
    raise FileNotFoundError(
        ...
        "Run codet5p-770m_trainer.py first or set LORA_ADAPTER."
    )
...
int main(void) {
    Point p = {10, 5};
    Point q = {3, 9};
    printf("manhattan((%d,%d), (%d,%d)) = %d\\n", p.x, p.y, q.x, q.y, manhattan(p, q));
    return 0;
}
"""
prompt = c_code_snippet.strip()
